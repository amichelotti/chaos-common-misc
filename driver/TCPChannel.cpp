...
TCPChannel::TCPChannel(const std::string& _ip_port):AbstractChannel(_ip_port),socket(io_service),deadline(io_service),byte_read(0), read_result(boost::asio::error::would_block),command(true),timeout_arised(0)  {
...
void TCPChannel::read_handler(const boost::system::error_code& ec, std::size_t size){
	if(ec){
		DERR("error ec:%s, size:%d",ec.message().c_str(),size);
	} else {
...
void TCPChannel::check_deadline( )
 {
   ...
   deadline.async_wait(boost::bind(&TCPChannel::check_deadline, this));
 }

int TCPChannel::read(void *buff,int nb,int ms_timeo,int*td){
	if(socket.is_open()){
		...
		asio::async_read( socket,  boost::asio::buffer(buff, nb), boost::asio::transfer_all(),boost::bind(&TCPChannel::read_handler,this, boost::asio::placeholders::error,boost::asio::placeholders::bytes_transferred));
...
int TCPChannel::read_async(void *buff,int nb){
	boost::system::error_code ec = boost::asio::error::would_block;
	asio::async_read( socket,  boost::asio::buffer(buff, nb), boost::bind(&TCPChannel::read_handler,this,ec,byte_read));
...
int TCPChannel::write(void *buffer,int nb,int ms_timeo,int*timeout_arised){
	if(socket.is_open()){
		boost::system::error_code error;
		int ret;
